...
interface Character {
  ...
  function Page() {
    ...
    const [dateOfCreation, setDateOfCreation] = useState(new Date().toISOString());
    ...
    const updateCharacter = async (updatedCharacter: Character) => {
      try {
        await axios.put(`https://localhost:7178/Character/${updatedCharacter.id}`, updatedCharacter);
        loadData(); // recharger les données après une mise à jour
      } catch (error) {
        console.error(error);
      }
